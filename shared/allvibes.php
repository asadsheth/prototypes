...
// http://mobile-homerun-yql.media.yahoo.com:4080/api/vibe/v1/editorial/topics/66e9fb86-b27f-3261-9593-c47d4e841dd8
...
if(!isset($ALL_VIBES)) {
	$ALL_VIBES = array(
		...
		array( 'name' => 'Local News', 'id' => 'a4e6cc20-f784-3169-8d99-2e22d369a550' ),
		...
		array( 'name' => 'The Daily Digit', 'id' => 'ac087d58-5131-39d2-b363-f869c1d6b700'),
...
// continuously add new suggested vibes here
$NEW_SUGG_VIBES = get_suggested_vibes();
for($i = 0; $i < count($NEW_SUGG_VIBES); $i++)	{
	// does this vibe already exist?
	$not_found_this_vibe = true;
	for($j = 0; $j < count($ALL_VIBES) && $not_found_this_vibe; $j++)	{
		if($ALL_VIBES[$j]['id'] == $NEW_SUGG_VIBES[$i]['id']) {
			$not_found_this_vibe = false;
		}
	}

	// no!
	if($not_found_this_vibe) {
		array_push($ALL_VIBES, array(
			'name' => $NEW_SUGG_VIBES[$i]['name'],
			'id' => $NEW_SUGG_VIBES[$i]['id']
		));
	}
}
...
for($i = 0; $i < count($NEW_ADD_VIBES); $i++)	{
	...
	$not_found_this_vibe = true;
	for($j = 0; $j < count($ALL_VIBES) && $not_found_this_vibe; $j++)	{
		if($ALL_VIBES[$j]['id'] == $NEW_ADD_VIBES[$i]['id']) {
			$not_found_this_vibe = false;
		}
	}
	...
	if($not_found_this_vibe) {
		array_push($ALL_VIBES, array(
			'name' => $NEW_ADD_VIBES[$i]['name'],
			'id' => $NEW_ADD_VIBES[$i]['id']
		));
	}
}

// add ntk and mega last; only add megastream video if we really really want it
// array_push($ALL_VIBES, 	array( 'name' => 'Trending', 'id' => '@MEGASTREAMVIDEO' ));
// array_push($ALL_VIBES, 	array( 'name' => 'Trending', 'id' => '@MEGASTREAM' ));
array_push($ALL_VIBES, 	array( 'name' => 'Headlines', 'id' => '@NTKVIDEO' ));
